...
-- fmap (\x -> sigmonid x ) (multStd theta vectors)

--costFunction1 vectors labels theta =
 --    trace (multStd (fmap (\x -> -x) labels) (fmap (2+)  ((fmap (\x -> sigmoid x ) (multStd theta vectors))))) - trace (multStd (fmap (\x -> 1-x) labels) (fmap (2+) (fmap (\x -> 1-x ) ((fmap (\x -> sigmoid x ) (multStd theta vectors)))))) / (nrows vectors)

costFunction1 vectors labels theta =
    (trace (multStd (fmap (\x -> -x) labels) (fmap log  ((fmap (\x -> sigmoid x ) (multStd theta vectors))))) - trace (multStd (fmap (\x -> 1-x) labels) (fmap log (fmap (\x -> 1-x ) ((fmap (\x -> sigmoid x ) (multStd theta vectors)))))) ) / 1
